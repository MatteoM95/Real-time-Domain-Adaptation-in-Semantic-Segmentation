    ...
    with torch.no_grad():
        ...
        for i, (data, label) in enumerate(dataloader):
            ...
            if i < 4:
                ...
                current_image.save("prova/" + str(i) + "-" + str(epoch) + "_camvidOriginal.png")
                current_label = colorize_mask(np.asarray(label, dtype=np.uint8))
                current_label.save("prova/" + str(i) + "-" + str(epoch) + "_label.png")
                current_predi = colorize_mask(np.asarray(predict, dtype=np.uint8))
                current_predi.save("prova/" + str(i) + "-" + str(epoch) + "_predict.png")
